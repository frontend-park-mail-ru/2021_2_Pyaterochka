...
class CreatorView extends Component {
    ...
    render() {
        ...
        element.querySelector(".creator-card").replaceWith(creatorCard.render());
        ...
        this.attributes.levels.map(card => new LevelCard(card)).forEach(card => {
            levelCardContainer.appendChild(card.render())
        });
        ...
        this.attributes.posts.map(card => new PostCard(card)).forEach(card => {
            postCardContainer.appendChild(card.render())
        });
        ...
        element.appendChild(lockMessage.render())
        return element
    }
}
